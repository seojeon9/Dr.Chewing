...
## 프로젝트 개요
+ 현재 소비자물가가 상승함에 따라 외식비도 높은 상승률을 보이고 있다. 서울시와 각 기초자치단체에서는 물가안정을 위해 가격이 저렴하고 서비스가 좋은 모범업소를 선정하고 인센티브를 제공하는 <b>'착한가격업소'</b>라는 제도를 실시하고 있다.<br>
+ 이러한 착한가격업소의 운영현황과 소비자 만족도 분석을 통해 제도의 본 취지와 방향성에 맞게 운영되고 있는지 살펴보고자 한다.
...
- 착한가격업소의 이점인 '가성비'를 점수화해 각 구별 순위를 나타내었고 행정구별로 쉽게 찾을 수 있도록 지도에 표현하였다.
